      ...
      >
        ...
        <div className="w-full flex-1 flex items-center  justify-center mb-4 relative z-10 p-6">
          <img
            src={sponsor.image}
            alt={sponsor.name}
            className={`max-h-32 w-auto object-cover  transition-all duration-500 ${
        ...
        <div className="p-4 w-full relative z-10">
          <h3 className="text-xl font-semibold mb-2 text-center text-white group-hover:text-primary transition-colors duration-300">{sponsor.name}</h3>
          {sponsor.field && (
          ...
          {sponsor.description && (
            <p className={`text-muted-foreground text-sm text-center mt-2 transition-opacity duration-300 ${
              isHovered ? 'opacity-100' : 'opacity-0 max-h-0'
            } overflow-hidden ${isHovered ? 'max-h-24' : ''}`}>
              {sponsor.description}
            </p>
          )}
        </div>
...
const SponsorsSection: React.FC<SponsorsSectionProps> = ({ sponsors }) => {
  ...
  return (
    <section id="sponsors" className="py-24 bg-gray-900 relative">
